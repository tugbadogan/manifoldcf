...
public class KafkaOutputConnector extends org.apache.manifoldcf.agents.output.BaseOutputConnector {
     ...
     * configuration and specification in such a way that if two such strings
     * are equal, the document will not need to be sent again to the output data
     * store.
     */
    @Override
     ...
     * connector to flush any internal data structures it has been keeping
     * around, or to tell the output repository that this is a good time to
     * synchronize things. It is called whenever a job is either completed or
     * aborted.
     *
     * @param activities is the handle to an object that the implementer of an
     * output connector may use to perform operations, such as logging
